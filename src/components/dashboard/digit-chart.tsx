...
export function DigitChart() {
  ...
  const digitStrengths = useMemo(() => {
    ...
    currentMarket.lastDigits.forEach((d) => {
      if (d >= 0 && d <= 9) counts[d]++;
    });
    ...
    return counts.map((count, digit) => ({
      digit,
      strength: total > 0 ? (count / total) * 100 : 0,
      count,
    ...
    <div className="w-full max-w-4xl mx-auto px-4 mt-4">
      <div className="rounded-2xl border border-white/[0.06] bg-gray-900/60 backdrop-blur-xl p-4 sm:p-5">
        ...
        <div className="flex items-end justify-between gap-1.5 h-32 px-2">
          {digitStrengths.map((d) => {
            const height = (d.strength / maxStrength) * 100;
            return (
              <div key={d.digit} className="flex-1 flex flex-col items-center gap-1.5">
                  ...
                  <div
                    className={`w-full rounded-t-sm transition-all duration-500 ${
                      d.isTop
                        ...
                        : "bg-gray-700/60"
                    } ${d.isTop ? "shadow-lg shadow-emerald-500/20" : ""}`}
                ...
                <div className="flex items-center gap-0.5">
                  ...
                  {d.isTop && (
                    <Star size={8} className="text-emerald-400 fill-emerald-400" />
                  )}
                </div>
              </div>
            );
